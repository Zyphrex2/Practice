...
# was written by Mr. John Schram 4/27/18
...
def drawPolygon(points):   
   if len(points) < 6:
      ...
      print("you must have at least 6 int parameters.")
      exit()
   elif len(points) % 2 != 0:
      update()
      sleep(1)
      print("Polygon Error in line",inspect.currentframe().f_back.f_back.f_lineno)
      print("When using drawPolygon/fillPolygon")
      print("you must have an even # of int parameters.")
      exit()
   else:
...
# The first 4 parameters are identical to <drawOval>.
...
# The first 4 parameters are identical to <fillOval>.
# The last 2 indicate the degree measure of where 
# the arc begins (start) and ends (finish).
# NOTE: 0 degrees is at the 12:00 position.
# ALSO: The arc is drawn clockwise.
...
# The final parameter (numLines) determines the number
...
# The final parameter (spacing) controls the
...
# Precondition: All 3 parameters are integers in the range [0..255] 
# NOTE: If only one parameter is used, and it is a number, it will 
#       determine the color based on a preselected list.  
# ALSO: If only one parameter is used, and it is a string, it will 
